...
float[] RK4(float curX, float curY){
  float a = curX*uwnd.getColumnCount() / width;
  float b = curY*uwnd.getRowCount() / height;
  
  float[] newPos = new float[2];
  float dx1 = readInterp(uwnd, a, b)*h;
  float dy1 = -readInterp(vwnd, a, b)*h;
  
  float k1X = dx1;
  float k1Y = dy1;
  
  float mid1X = (curX+h*k1X/2)*uwnd.getColumnCount() / width;
  float mid1Y = (curY+h*k1Y/2)*uwnd.getRowCount() / height;
  
  float k2X = readInterp(uwnd, mid1X, mid1Y);
  float k2Y = readInterp(vwnd, mid1X, mid1Y);
  
  float mid2X = (mid1X + h*k2X/2)*uwnd.getColumnCount() / width;
  float mid2Y = (mid1Y + h*k2Y/2)*uwnd.getRowCount() / height;
  
  float k3X = readInterp(uwnd, mid2X, mid2Y);
  float k3Y = readInterp(vwnd, mid2X, mid2Y);
  
  float mid3X = (mid2X + h*k3X)*uwnd.getColumnCount() / width;
  float mid3Y = (mid2Y + h*k3Y)*uwnd.getRowCount() / height;
